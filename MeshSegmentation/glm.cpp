...
glmFindMaterial(GLMmodel* model, char* name)
{
  ...
  printf("\nglmFindMaterial():  can't find material \"%s\".", name);
  i = 0;
...
glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
	...
	break;
      default:
	printf("\nglmFirstPass(): Unknown token \"%s\".", buf);
	exit(1);
	break;
      }
      break;
  ...
  printf("\n Normals: %d", numnormals);
  printf("\n Texcoords: %d", numtexcoords);
  printf("\n Triangles: %d", numtriangles);
  printf("\n Groups: %d", model->numgroups);
#endif
...
glmDimensions(GLMmodel* model, GLfloat* dimensions)
{
  ...
  /* calculate model width, height, and depth */
  dimensions[X] = glmAbs(maxx) + glmAbs(minx);
  dimensions[Y] = glmAbs(maxy) + glmAbs(miny);
  dimensions[Z] = glmAbs(maxz) + glmAbs(minz);
}
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
  ...
  printf("\nglmVertexNormals(): %d normals generated", model->numnormals);
}
...
glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  if (mode & GLM_SMOOTH && !model->normals) {
    printf("\nglmDraw() warning: smooth render mode requested "
	   "with no normals defined.");
    mode &= ~GLM_SMOOTH;
  }
  if (mode & GLM_TEXTURE && !model->texcoords) {
    printf("\nglmDraw() warning: texture render mode requested "
	   "with no texture coordinates defined.");
    mode &= ~GLM_TEXTURE;
  }
  if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
